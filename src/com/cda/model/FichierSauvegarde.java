...
public class FichierSauvegarde {
	...
	private static File vFichier = new File("c:\\temp\\sauvegarde-jeu.txt");
	...
	public static void fichier() {
		...
		try (FileWriter vFileWriter = new FileWriter(vFichier, true)) {
			vFileWriter.append(Accueil.nomJoueur2).append(" ").append(String.valueOf(GestionCollision.compteurGlobal))
					.append(" ").append(vDateFormater).append("\n");
			vFileWriter.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
